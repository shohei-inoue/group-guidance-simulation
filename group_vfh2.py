...
# -------------------------------- const parameter ---------------------------------------------------
MAX_MOVEMENT = 5.0                          # 最大移動量
MIN_MOVEMENT = 3.0                          # 最小移動量
MAX_BOIDS_MOVEMENT = 5.0                    # boids判定時の最大移動量
MIN_BOIDS_MOVEMENT = 3.0                    # boids判定時の最小移動量
...
# ------------ VFHに使用するパラメータ -----------------------
VFH_DRIVABILITY_BIAS = 0.8
VFH_EXPLORATION_BIAS = 0.2
...
def main():
    ...
    while True:
        ...
        for i in range(group_num):
            ...
            # 探査中心の変換(ステップ数で変換ver)
            if red_list[i][-1].step % 200  == 0:
                if main_marker_list[i].coverage_ratio >= 80.0:

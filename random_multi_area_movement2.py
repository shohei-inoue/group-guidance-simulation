# 複数群での戻り(探査中心移動方向と逆方面)を入れたランダムなフロンティアベースの誘導
import random
import math
import numpy as np
import pandas as pd
    ...
    while True:
        ...
        for i in range(group_num):
            ...
            if main_marker_list[i].coverage_ratio >= AREA_COVERAGE_THRESHOLD:
                ...
                # =================== add ================
                if main_marker_list[i].x != main_marker_list[i].parent.x:
                    vec_d = np.array(main_marker_list[i].point - main_marker_list[i].parent.point)
                    vec_x = np.array([main_marker_list[i].x - main_marker_list[i].parent.x, 0])
                    marker_azimuth = np.rad2deg(math.acos(vec_d @ vec_x / (np.linalg.norm(vec_d) * np.linalg.norm(vec_x))))
                
                if main_marker_list[i].x - main_marker_list[i].parent.x < 0:
                    if main_marker_list[i].y - main_marker_list[i].parent.y >= 0:
                        marker_azimuth = np.rad2deg(math.pi) - marker_azimuth
                    else:
                        marker_azimuth += np.rad2deg(math.pi)
                else:
                    if main_marker_list[i].y - main_marker_list[i].parent.y < 0:
                        marker_azimuth = np.rad2deg(2.0 * math.pi) - marker_azimuth
                
                theta1 = marker_azimuth + np.rad2deg(math.pi / 2.0)
                if theta1 > np.rad2deg(2.0 * math.pi):
                    theta1 -= 2.0 * math.pi
                
                theta2 = marker_azimuth - 90.0
                if theta2 < 0:
                ...
                next_theta_list = []
                if main_marker_list[i].x - main_marker_list[i].parent.x < 0:
                    next_theta_list.append(random.uniform(0.0, np.deg2rad(theta2)))
                    next_theta_list.append(random.uniform(np.deg2rad(theta1), 2.0 * math.pi))

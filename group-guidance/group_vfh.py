...
def main():
    # コメントアウト以外1段ずつ字下げしている
    cov_df = pd.DataFrame({'num': [0], 'coverage_ratio': [0]})

    for loop in range(100):
        map_coverage_ratio = 0
        # 障害物マップの取得
        obstacle_list, obstacle_df_list, obstacle_size = obstacle.obstacle_map_exs2()
...
#            ax[-1].set_title('main grid_map')
    ...
    #    obstacle_df_list[i].to_csv(params.SAVE_DIRECTORY + 'obstacle' + str(i + 1) + '.csv')
        cov_df = pd.concat([cov_df, pd.DataFrame({'num': [loop + 1], 'coverage_ratio': [map_coverage_ratio]})])
        print("loop count : ", loop)

    cov_df.to_csv(params.SAVE_DIRECTORY + 'sim53_b_cov01.csv')

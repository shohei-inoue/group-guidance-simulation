...
class Pseudo_obstacle:
    ...
    def pseudo_obstacle_area_formation(self):
        ...
        self.center_point = np.array([self.center_x, self.center_y])
        if abs((max(point_X_list) - min(point_X_list)) / 2) <= 1:
            self.w = 1.0
        else:
            self.w = abs((max(point_X_list) - min(point_X_list)) / 2)
        if abs((max(point_Y_list) - min(point_Y_list)) / 2) <= 1:
            self.h = 1.0
        else:
            self.h = abs((max(point_Y_list) - min(point_Y_list)) / 2)
